...
* Consider the code snippet below that matches on an ```Option<u8>``` value in the ```config_max``` variable but only wants to execute code if the value is the ```Some``` variant:
```rust
...
```
* If the value is ```Some```, we print out the value in the ```Some``` variant by binding the value inside it to the variable ```max``` in the pattern. We don’t want to do anything with the ```None``` value. To satisfy the ```match``` expression, we have to add ```_ => ()``` after processing just one variant, which is annoying boilerplate code to add.
* Instead, we could write this in a shorter way using ```if let```. The following code behaves the same as the ```match``` expression above:
```rust
let config_max = Some(3u8);
...
```
* __IMPORTANT: Using ```if let``` means less typing, less indentation, and less boilerplate code. However, you lose the exhaustive checking that ```match``` enforces.__
...
* Comparing ```match``` and ```if let else```:
  1. ```match```:
  ```rust
  let mut count = 0;
  match coin {
